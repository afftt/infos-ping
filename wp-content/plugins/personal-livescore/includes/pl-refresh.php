...
// display live scores ------- this condition is used to determine if $ rel exists. It may exist, but be empty.
if(isset($_GET['rel'])) {
    $rel = $_GET['rel'];
    // Now, I look if rel isn't empty. If isn't, so I apply your first request.
    if($rel != '')
    {
    	$results = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "livescore WHERE pl_active=1 AND pl_id=" . $rel . " ORDER BY pl_datetime ASC", ARRAY_A);
	}
	// And if it is empty I apply your second request.
	else {
    $results = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "livescore WHERE pl_active=1 ORDER BY pl_datetime ASC", ARRAY_A);
	}

// I think this is the best way. I have tried with a MySQL and a MariaDB and it works with the shortcode : [livescore]
}
