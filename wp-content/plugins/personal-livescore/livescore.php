...
Plugin URI: http://getbutterfly.com/wordpress-plugins/personal-livescore/
...
define('PL_PLUGIN_PATH', WP_PLUGIN_DIR . '/' . dirname(plugin_basename(__FILE__)));
define('PL_VERSION', '2.4.1');
//
...
function setOptionsPL() {
	...
	add_option('pl_copyright_line', 'Widget Livescore &copy;2014 <a href="#">Your Site</a>');
...
function load_livescore_wp_admin_style() {
	wp_register_style('pl-calendar', PL_PLUGIN_URL . '/css/jCalendar.css', false, '1.0.0');
	wp_enqueue_script('pl-calendar-script', plugins_url('/js/jCalendar.js', __FILE__));
	wp_enqueue_style('pl-calendar');
}
	...
	<div class="wrap">
		...
		<?php if($active_tab == 'gb_settings') { ?>
			...
			<div id="poststuff" class="ui-sortable meta-box-sortables">
						...
						<form method="post">
							...
							<table class="form-table">
								...
								<tr valign="top">
									...
									<td>
										<?php echo get_option('pl_showembed') == '1' ? '<input type="checkbox" name="pl_showembed" id="pl_showembed" value="1" checked="checked">' : '<input type="checkbox" name="pl_showembed" id="pl_showembed" value="1">'; ?>
				...
				<p><em>You are using <b>Personal Livescore</b> version <b><?php echo PL_VERSION; ?></b>.</em></p>
				<p>This plugin allows the administrator to run and maintain a livescore system without the need to sign up for various web services or feeds. The plugin is completely standalone.</p>
		...
		<?php } if($active_tab == 'gb_livescore_manager') { ?>
            ...
            if(isset($_POST['gb_livescore_go'])) {
				...
				echo '<div class="updated"><p>Score updated successfully!</p></div>';
			}
			if(isset($_POST['gb_livescore_new'])) {
				...
				echo '<div class="updated"><p>Score added successfully!</p></div>';
			}
			?>
			<div id="poststuff" class="ui-sortable meta-box-sortables">
				<div class="postbox">
					...
					<div class="inside">
								...
								echo '<form method="post">';
									echo '<tr>';
										...
										echo '<td>
											...
											<label><img onclick="viewCalendar(\'jcalendar_parent\', \'pl_datetime\')" width="16" height="16" src="' . PL_PLUGIN_URL . '/images/icon-calendar.png" alt="" class="pl-calendar"></label>
											<div id="jcalendar_parent" class="jcalendar_parent" style="z-index: 999"></div>
										</td>';
										echo '<td><input type="text" name="pl_sport" placeholder="Sport" size="8"></td>';
										echo '<td><input type="text" name="pl_location" placeholder="Location" size="8"></td>';
										echo '<td>
											<input type="text" name="pl_team1" placeholder="Team 1" size="12"><label> vs </label>
											<input type="text" name="pl_team2" placeholder="Team 2" size="12">
										</td>';
										echo '<td>
											<input type="number" min="0" max="99" step="1" name="pl_score1" value="0"><label>:</label>
											<input type="number" min="0" max="99" step="1" name="pl_score2" value="0">
										</td>';
										echo '<td><input type="number" min="0" max="9" step="1" name="pl_set" value="0"></td>';
										echo '<td><input type="number" min="0" max="3" step="1" name="pl_active" value="1"></td>';
								...
								foreach($results as $row) {
									echo '<form method="post">';
										echo '<tr>';
											...
											echo '<td><input type="datetime" name="pl_datetime" value="' . $row['pl_datetime'] . '" size="16"></td>';
											echo '<td><input type="text" name="pl_sport" value="' . $row['pl_sport'] . '" size="8"></td>';
											echo '<td><input type="text" name="pl_location" value="' . $row['pl_location'] . '" size="8"></td>';
											echo '<td>
												<input type="text" name="pl_team1" value="' . $row['pl_team1'] . '" size="12"><label> vs </label>
												<input type="text" name="pl_team2" value="' . $row['pl_team2'] . '" size="12">
											</td>';
											echo '<td>
												<input type="number" min="0" max="99" step="1" name="pl_score1" value="' . $row['pl_score1'] . '"><label>:</label>
												<input type="number" min="0" max="99" step="1" name="pl_score2" value="' . $row['pl_score2'] . '">
											</td>';
											echo '<td><input type="number" min="0" max="9" step="1" name="pl_set" value="' . $row['pl_set'] . '"></td>';
											...
											echo '<td>
												...
												<input type="button" name="gb_livescore_goals" value="&rarr;" class="button button-secondary" onclick="location.href=\'' . admin_url('options-general.php?page=pl&tab=gb_goal_manager&matchid=' . $row['pl_id']) . '\'">
											</td>';
										echo '</tr>';
									echo '</form>';
								}
						...
						?>
						<p><small><b>Reminder:</b> (Status) <b>0</b> is an archived game, <b>1</b> is live, <b>2</b> is upcoming and <b>3</b> is upcoming but hidden. Delete games by archiving them and removing them from the <img src="<?php echo PL_PLUGIN_URL; ?>/images/icon-tab.png" alt="" class="inner-icon"> <b>Archive</b> tab.</small></p>
					</div>
				</div>
			</div>
		<?php } if($active_tab == 'gb_goal_manager') { ?>
			...
			if(isset($_GET['matchid'])) {
				$match_id = mysql_real_escape_string($_GET['matchid']);
				...
				if(isset($_POST['gb_goal_edit'])) {
					...
					echo '<div class="updated"><p>Goal updated successfully!</p></div>';
				}
				...
				if(isset($_POST['gb_livescore_goal'])) {
					...
					echo '<div class="updated"><p>Goal added successfully!</p></div>';
				}
				?>
				<div id="poststuff" class="ui-sortable meta-box-sortables">
					<div class="postbox">
								...
								foreach($results as $row) {
									...
									echo '<form method="post">';
										echo '<tr' . $pl_visibility . '>';
											echo '<td><input type="datetime" name="pl_datetime" value="' . $row['pl_datetime'] . '"></td>';
											echo '<td><input type="text" name="pl_sport" value="' . $row['pl_sport'] . '" size="8"></td>';
											echo '<td><input type="text" name="pl_location" value="' . $row['pl_location'] . '" size="8"></td>';
											echo '<td>
												<input type="text" name="pl_team1" value="' . $row['pl_team1'] . '" size="12"><label> vs </label>
		...
		<?php } if($active_tab == 'gb_livescore_archive') { ?>
			...
			if(isset($_POST['gb_livescore_del'])) {
				...
				echo '<div class="updated"><p>Score deleted successfully!</p></div>';
			}
			?>
			<div id="poststuff" class="ui-sortable meta-box-sortables">
				<div class="postbox">
...
function livescore_display($atts) {
	...
	if($mode == 'archive') {
		...
		$display .= '<div class="pl_archive">';
			$display .= '<div class="pl-header"><span class="pl-category">' . get_option('pl_label_archive') . '</span></div>';
            foreach($results as $row) {
				if($row['pl_set'] != 0) $pl_set = ' <span class="pl-set">SET ' . $row['pl_set'] . '</span>';
				$display .= '<div id="row">';
					$display .= '<div class="pl-details">' . $row['pl_sport'] . ', ' . $row['pl_location'] . ', ' . $row['pl_datetime'] . '</div>';
		...
		if(get_option('pl_showembed') == 1)
			$display .= '<div><textarea style="font-family: monospace; width: 100%;" rows="4">&lt;iframe src="' . PL_PLUGIN_URL . '/livescore-embed.php" width="100%" height="600"&gt;&lt;/iframe&gt;</textarea></div>';
	}

	return $display;
}
add_shortcode('livescore', 'livescore_display'); // shortcode, function

function addHeaderLinks() {
	$pl_refresh_interval = get_option('pl_refresh_interval');
	$pl_refresh_interval = $pl_refresh_interval * 1000;
	echo '<link rel="stylesheet" type="text/css" href="' . PL_PLUGIN_URL . '/css/style.css">';
	echo '
	<script>
