...
if ( !class_exists( 'RWMB_Map_Field' ) )
{
	class RWMB_Map_Field
	{
		...
		static function html( $html, $meta, $field )
		{
			...
			{
				$html .= sprintf(
					'<button class="button" style="visibility:hidden;" type="button" id="rwmb-map-goto-address-button" value="%s" onclick="geocodeAddress(this.value);">%s</button>',
					is_array( $address ) ? implode( ',', $address ) : $address,
					__( 'Find Address', 'mtcframework' )
				);
			}
			return $html;
		}
