...
class mtc_recent_post_widget extends WP_Widget {

    function mtc_recent_post_widget() {
		# Constructor
		$widget_ops = array( 'classname' => 'mtc_recent_post_widget', 'description' => __( 'Widget recent post with media' ,'mtcframework') );
		$this->WP_Widget( 'mtc-recent-post', __( 'MTC : Recent Post with media' ,'mtcframework'), $widget_ops );
	} 	# end of function mtc_recent_post_widget()
    ...
    function form($instance) {
		...
		$title 		= (!empty($instance['title'])) ? esc_attr($instance['title']) : 'Recent Post' ;
		$limit 		= (!empty($instance['limit'])) ? esc_attr($instance['limit']) : 4 ;
        ?>
		<p>
...
add_action( 'widgets_init', create_function( '', 'return register_widget("mtc_recent_post_widget");' ) );
